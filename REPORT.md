...
## Putting this in perspective — what it says about V-JEPA's claims

V-JEPA's central architectural bet — restated in the V-JEPA2 paper's own title, *"Self-Supervised Video Models Enable Understanding, Prediction and Planning"* — is that training a model to predict in **latent space**, rather than reconstructing pixels, forces it to discard irrelevant texture/noise and retain the physically/semantically meaningful structure of a scene, without labels. This experiment's numbers map onto three separable pieces of that claim, plus one check on what produced them:

| Claim component | What would support it | Our number | Verdict |
|---|---|---|---|
| The latent space organizes a physical variable as a shared, reusable direction — not scene-specific noise | Consistent cosine alignment across independent scenes | A = 0.635 over 780 pairs, only 1 negative | **Supports** — real, moderate-to-strong abstraction, not a perfectly crisp single axis (well under the 1.0 ceiling) |
| That structure is specific to the physical intervention, not "any embedding difference looks similar" | Low/near-zero alignment with an unrelated (appearance) intervention | Matched gravity–color cosines average ≈0.029 (near-orthogonal); D = 0.598 | **Supports** — the space at least partially factorizes physical change from appearance change |
| The structure is usable for actual generalizable prediction, not just descriptive of the training scenes | A direction fit on some scenes correctly predicts the shift in scenes it never saw | R = 0.601 (39.9% RMSE reduction), f = 1.0 (10/10 held-out scenes individually improved) | **Supports** — closest of the three to a literal "predictive" claim: simple vector arithmetic on the latent space forecasts a physical counterfactual |
| This is a property of V-JEPA2's *training*, not just architecture or trivial pixel geometry | The signal disappears when weights are untrained, or the model is removed entirely | Random-weight control: A≈0, D≈0, R≈1 (worse than baseline). Pixel-statistics control: same collapse | **Supports** — ties the effect specifically to what the self-supervised latent-prediction objective learned, not to having a 768-dim cosine space at all |

**What this does *not* test.** V-JEPA2's own headline "prediction and planning" results (robot-arm planning, next-state forecasting) exercise a *different* component: the trained **predictor** network, which forecasts masked/future latent states from partial context. This experiment only ever ran the frozen **encoder** on complete, already-rendered clips — the predictor module was never invoked. So this result is evidence that V-JEPA2's encoder learns a structured, physically-relevant, generalizable representation — a *necessary precondition* for the predictor to have something useful to forecast — not a direct replication of the forward-simulation/planning claim itself.

**Reading the magnitudes honestly.** None of these numbers are saturated. A=0.635 and D=0.598 sit well below the 1.0 ceiling — there is real per-scene variability, not one perfectly crisp shared axis. R=0.601 still leaves substantial residual error (RMSE₁=0.419, not 0) — a 40% reduction, not a solved prediction problem. The honest summary: this encoder's latent space is **usefully and non-trivially structured** around this physical variable — not **perfectly or deterministically** so — and that structure demonstrably required V-JEPA2's actual pretraining, not just its architecture.
